sap.ui.define([
    ...
    return {
        ...
        onValueHelpRequested: function (oEvent, that, sPath, aVHFields) {
            ...
            that.loadFragment({
                name: "pp.fglabel.fragments.ValueHelpDialog"
                ...
                var sCon = this.getView().getControllerName();
                if (sCon === "pp.fglabel.controller.Generate") {
                    var sPlant = that.getModel("local").getProperty("/PlantG");
                } else {
                    var sPlant = that.getModel("local").getProperty("/Plant");
                };
                var aFilters = [];
                if (sPath === "/ZC_WO_ORDERVH") {
                    if (sPlant) {
